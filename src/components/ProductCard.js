...
const ProductCard = ({title,image,price})=> {
  ...
  return(
    <div data-aos-once="true" data-aos="fade-right" className="w-[48%]  flex flex-col min-w-[48%]  relative">
    <Link className="" to={`/product/${title.replace(/ /g,"-")}`}>
        <span className=" bg-red-500 text-white text-sm px-2 rounded-lg absolute top-2 left-2">8% OFF</span>
            ...
            <h3 className="font-medium text-sm mt-1 line-clamp-2 h-[15%]">{title}</h3>
